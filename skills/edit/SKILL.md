...
**Spawn a section-writer subagent** for each section being revised. The section-writer runs the full skill pipeline:

```
Agent: section-writer
Input: { section (with current text + edit instructions), sectionIndex, thesis, styleFingerprint, citationStyle, runId, tools, priorSectionTexts }
```

The section-writer applies: draft revision → style compliance → Hebrew grammar → repetition check → citation audit.

For multiple sections being edited, **spawn all section-writer agents in parallel**.

### Mode B: Citation Fix

**Spawn auditor subagents in parallel** — one per section that needs citation work:
...
5. After restructuring, **spawn synthesizer** to fix transitions:
...
**Spawn the synthesizer agent** on the complete article:
