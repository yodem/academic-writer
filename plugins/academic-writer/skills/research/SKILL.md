...
If tools.hybrid-search-rag.enabled:
  ...
  → Runs multi-mode RAG queries (mix + local + bypass as needed)

If tools.candlekeep.enabled:
  Agent: research-candlekeep
  subagent_type: general-purpose
  Input: { question, questionType, sourceIds }
  → Searches and reads Candlekeep documents directly

If tools.mongodb-agent-skills.enabled:
  (Query MongoDB directly — lightweight, no subagent needed)
  → Search collections, retrieve annotations/notes
```

**Launch the RAG and Candlekeep agents in parallel using the Agent tool.** Each runs independently and returns its findings.
